...
When connected over MCP, check tools/list for open_upload_panel and get_uploads. Prefer the panel when the person needs to supply photos: open it, show the inline panel or browser link, wait for them to finish, then retrieve ready image URLs with get_uploads. Pass the returned URLs into the existing CLI/REST image fields without making permanent copies. Images expire 24 hours after the panel is created: respect expires_at and ask for a new upload if expired. Still PNG/JPEG/WebP, 25 MB each, ten per panel. No base64, shell, or external image hosting required from the user. If these tools are absent, use the existing upload flow for files you can actually access; never invent access to chat attachments.
...
| **crazy-look** | Silent extreme close-up reaction clip with a static caption overlay ("the crazy look"). | [`reference/generators/crazy_look.md`](reference/generators/crazy_look.md) |
...
- [`reference/generators/crazy_look.md`](reference/generators/crazy_look.md), Silent extreme close-up reaction clip with a static caption overlay ("the crazy look").
