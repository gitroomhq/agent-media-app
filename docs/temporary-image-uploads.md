...
The integration uses the shared MCP Apps `_meta.ui.resourceUri`, `text/html;profile=mcp-app`, resource discovery, and SDK bridge. `Use images in chat` updates model context and sends a user message only on the person's click. After a batch adds images, an automatic modal offers **Copy prompt** and asks the person to paste it into Claude or ChatGPT. The prompt includes the session ID, original image URLs, expiry, and instructions to retrieve and inspect the previews before using the originals for generation. It contains no upload capability token. The modal opens once per completed batch, can be dismissed to add more images, and offers manual copying if clipboard access is blocked. A host that cannot send a message falls back to copying links. Actual panel availability depends on the client's MCP Apps support and connector configuration; the browser handoff remains usable by any agent that can call the authenticated HTTP or MCP tools.
