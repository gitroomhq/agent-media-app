// @ts-nocheck — vendored shader component from reactbits.dev/animations/laser-flow.
// The runtime is verified visually; type-checking the WebGL/Three.js internals
// adds noise without value since the component is treated as an opaque black box.
...
void mainImage(out vec4 fc,in vec2 frag){
    ...
    // Huly-style three-bucket color split:
    //   - Core beam (L) → pure white, ignores tint
    //   - Wisps + fog (w + fog) → tinted by uColor (dark purple)
    // Splitting the fog out of the beam bucket keeps the bottom spread
    // and surrounding clouds in the purple atmosphere colour instead of
    // bleaching out to white.
    // Huly-style: keep the original combined intensity for brightness,
    // but blend the colour from uColor (dark purple — atmosphere/wisps)
    // toward pure white as intensity ramps up (so the bright beam core
    // bleaches to white while the dim particles stay tinted).
    float tone = g(LF + w);
    float whitenAmt = smoothstep(0.35, 1.1, tone);
    vec3 tint = mix(uColor, vec3(1.0, 1.0, 1.0), whitenAmt);
    vec3 col = tone * tint + dith;
...
export const LaserFlow = ({
  ...
  useEffect(() => {
    ...
    // Creating a WebGL context throws on bots (WebPageTest etc.) and very old /
    // headless devices that have no WebGL. That used to crash to Sentry as
    // "Error creating WebGL context" (~65 events). The hero is a decorative
    // overlay, so on failure we degrade gracefully: skip the effect, leave the
    // container empty, and never throw. (2026-06-12)
    let renderer;
    try {
      ...
      // No WebGL available — render nothing rather than crash.
      return;
    }
