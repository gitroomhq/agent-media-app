// @ts-nocheck
// Copyright 2026 react-bits contributors. Original: https://reactbits.dev/backgrounds/liquid-ether
// Vendored from DavidHDev/react-bits, MIT licensed.
...
export default function LiquidEther({
  ...
  // - noInteract: when true, the mouse is completely ignored: no
  //   listeners are bound and the auto-driver never pauses on hover.
  autoMargin = 0.2,
  noInteract = false
}) {
  ...
  useEffect(() => {
    ...
    // Pre-flight WebGL check. On bots / very old / headless devices that have
    // no WebGL, constructing THREE.WebGLRenderer below throws "Error creating
    // WebGL context", which (thrown from this effect) trips the page-level
    // error boundary and replaces the WHOLE landing page with "Something went
    // wrong". This is a decorative background, so if WebGL is unavailable we
    // skip the entire effect and render nothing. (2026-06-12)
    try {
    ...
    class AutoDriver {
      constructor(mouse, manager, opts) {
        ...
        this.margin = typeof opts.margin === 'number' ? opts.margin : 0.2;
        this.ignoreHover = !!opts.ignoreHover;
      ...
      update() {
        ...
        if (!this.ignoreHover && idle < this.resumeDelay) {
          if (this.active) this.forceStop();
          return;
        }
        if (!this.ignoreHover && this.mouse.isHoverInside) {
          if (this.active) this.forceStop();
          return;
        }
        if (!this.active) {
    ...
    class WebGLManager {
      constructor(props) {
        ...
        this.autoDriver = new AutoDriver(Mouse, this, {
          ...
          margin: props.autoMargin,
          ignoreHover: props.noInteract
        });
        this.init();
