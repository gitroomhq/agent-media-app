...
/**
 * POST /api/v1/show-your-app
 *
 * Server-side proxy from the web dashboard to api-v2's
 * /v1/generate/show_your_app endpoint. Uses the user's Supabase
 * session to authenticate (api-v2 accepts JWTs).
 *
 * Keeps the call same-origin so the web's CSP doesn't have to list
...
export async function POST(req: NextRequest) {
  ...
  try {
    const upstream = await fetch(`${API_V2_URL}/v1/generate/show_your_app`, {
      method: 'POST',
      headers: {
