...
/**
 * POST /api/v1/character/sheet-generate
 *
 * Same-origin proxy from the dashboard to api-v2 — passes the user's
 * Supabase session as a Bearer token. Used by the Content Machine
 * wizard step 1 (Describe / Upload / Pick → character sheet).
 */
...
export async function POST(req: NextRequest) {
  ...
  try {
    const upstream = await fetch(`${API_V2_URL}/v1/character/sheet-generate`, {
      method: 'POST',
      headers: {
