...
export function looseSkillBody(repoRoot: string): string {
  ...
  return [
    ...
    '2. **Pick the model.** Omit `model` and you get the default (' + `\`${V2_DEFAULT_MODEL.video}\`` + ' for video, `' + V2_DEFAULT_MODEL.image + '` for images, `' + V2_DEFAULT_MODEL.audio + '` for speech); pass `"auto"` to let recent results decide. Call `list_models` when the job is unusual — it says what each model is good at, bad at, what it costs, and how it has actually performed lately. Only live models are accepted; naming a planned one returns the live list.',
