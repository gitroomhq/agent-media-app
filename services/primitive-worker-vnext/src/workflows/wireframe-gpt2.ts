...
import type {
  WireframeGpt2ActivityInput,
  WireframeGpt2ActivityResult,
} from '../activities/wireframe-gpt2.js';

const { wireframeGpt2 } = proxyActivities<PrimitiveActivities>({
  // startToClose >= OpenAI client timeout (now 300s) + overhead (Bug 4).
...
export async function wireframeGpt2Workflow(
  input: WireframeGpt2ActivityInput,
): Promise<WireframeGpt2ActivityResult> {
  try {
    return await wireframeGpt2(input);
  } catch (err) {
